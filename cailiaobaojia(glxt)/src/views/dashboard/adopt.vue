...
<div>
  ...
  <el-dialog title="提示" :visible.sync="dialogVisible" width="60%">
    <userInfo audit="2" ref="userItems" :user-id="userId"></userInfo>
...
export default {
  ...
  methods: {
    ...
    updateData() {
      console.log(this.$refs.userItems);
      return new Promise((resolve, reject) => {
        updateUserAudit({
          userId: this.userId,
          items: this.$refs.userItems.infoData ,
          projectIds:this.$refs.userItems.companyId.join(",")
        }).then(res => {
          if (res.status === 200) {
            this.$message({
